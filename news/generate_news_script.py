...
def repair_garbled_byte_tokens(text):
    # 特定の漢字(実測: "錮")が毎回同じ壊れ方をするケースがあり、
    # リトライしても同じ文字を使いたがる話題(例: 禁錮刑に言及するニュース)
    # では3回とも同じ理由で失敗し続けていた。<0xXX>の連続は元々1文字分の
...
def build_script_prompt(main_article, related_articles, web_results):
    ...
    if is_thin:
        ...
        length_instruction = (
            f"・{TARGET_SCENES}シーン程度になるよう、軸記事の詳細(仕様・価格等)を"
            "序盤で扱った後、関連材料(社内DB類似記事・Web検索結果)に実際に書かれている"
            "具体的な事実(競合製品との比較、関連する技術・規格の解説、市場動向、"
            "価格帯の比較等)を追加のシーンとして積極的に取り入れて厚みを持たせること。"
            "関連材料に事実として書かれている内容を使うのはハルシネーションにはあたらない"
            "ため、遠慮せず活用してよい。シーン数を埋めるためだけの一般論"
            "(「〜にも活用できます」等、どの材料にも書かれていない創作)は禁止する"
        )
